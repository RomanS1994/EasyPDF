...
import { refs as accountRefs } from '../auth/account/refs.js';
...
import { state as guestState } from '../auth/guest/state.js';
import {
...
import { renderOrderList } from '../auth/orders/orders.js';

function getSelectedManagerContextUser() {
  return (
...
export function renderManagerPlanOptions() {
  const plans = state.managerPlans.length ? state.managerPlans : guestState.plans;
...
export function renderManagerSelectedUser() {
  ...
  if (isAdminShell() && accountRefs.workspaceUsage) {
    accountRefs.workspaceUsage.textContent = user?.email || t('no_account');
  }
  ...
  const pendingPlanName = resolvePlanName(
    user.subscription?.pendingPlanId,
    state.managerPlans.length ? state.managerPlans : guestState.plans,
  );
  ...
  refs.managerRoleSection?.classList.toggle('is-hidden', accountState.user?.role !== 'admin');
...
export function renderManagerAccessState() {
  const canManage = isManagerRole(accountState.user?.role);
