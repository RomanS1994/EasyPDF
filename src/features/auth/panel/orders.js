...
function syncHistoryControls(activeTab, activeSort, visibleCount, dateFilterValue) {
  if (refs.statsHistorySummary) {
    refs.statsHistorySummary.textContent = t('orders_count', { count: visibleCount });
  }

  if (refs.statsHistorySortSelect && refs.statsHistorySortSelect.value !== activeSort) {
    refs.statsHistorySortSelect.value = activeSort;
  }

  if (refs.statsHistoryDateFilter && refs.statsHistoryDateFilter.value !== dateFilterValue) {
    refs.statsHistoryDateFilter.value = dateFilterValue;
  }
...
export function setOrdersDateFilter(dateValue = '') {
  ...
  if (refs.ordersDateFilterLabel) {
    refs.ordersDateFilterLabel.textContent = state.ordersDateFilter
      ? formatDateFilterLabel(state.ordersDateFilter)
      : t('all_dates');
  }
}
...
export function getFilteredOrders(orders = [], dateFilter = state.ordersDateFilter) {
...
export function renderOrderList(listElement, emptyElement, orders, options = {}) {
  ...
  const dateFilter = Object.prototype.hasOwnProperty.call(options, 'dateFilter')
    ? options.dateFilter || ''
    : isHistoryMode
      ? state.ordersHistoryDateFilter
      : state.ordersDateFilter;
  ...
  const historyTab = isHistoryMode ? normalizeHistoryTab(state.ordersHistoryTab) : 'all';
  const historySort = isHistoryMode ? normalizeHistorySort(state.ordersHistorySort) : 'newest';
