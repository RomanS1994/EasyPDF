...
import { categorizeOrderStatus, escapeHtml } from '../../../shared/lib/formatters.js';
import { activateStatsTab } from '../shell/routes.js';

export function getMetrics() {
  const usage = accountState.user?.usage || {
    periodStart: '',
    periodEnd: '',
    cycleLabel: '-',
    used: 0,
  ...
  const generatedOrders = accountState.orders.filter(order => order.status === 'pdf_generated').length;

  return {
    usage,
    totalOrders: accountState.orders.length,
    generatedOrders,
...
function buildActivitySeries(usage) {
  ...
  return Array.from({ length: 7 }, (_, index) => {
    ...
    const count = accountState.orders.filter(order => String(order.createdAt || '').slice(0, 10) === key).length;
...
function buildStatusBreakdown() {
  ...
  accountState.orders.forEach(order => {
